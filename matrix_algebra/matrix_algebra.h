...
public:
    ...
    //initializes a matrix with specified 1d array data must pass the size of the array
    ...
    ~matrix();//tested
    //assignment function if you want to overwrite on the same matrix or change its dimensions
    //aka assign a new matrix of new dimensions this allows for it into a matrix of different dimensions
    ...
    //write different values in a created matrix
    ...
    matrix operator*(matrix&)const;//tested
    //returns caller/ sent matrix uses @inverse and returns the result as a new matrix
    matrix operator/(matrix&)const;//tested
    ...
    //angle between caller and passed matrix
    ...
    //lpu where p is permutations that occured during factorization
    //lower_fact and permutaion & upper_fact have the results
    ...
    //and returns a row if the row is switched it returns the row that was switched (its index)
    //if its not switched it reurns same row
